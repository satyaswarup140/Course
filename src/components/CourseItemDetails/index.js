...
export default class CourseItemDetails extends Component {
  state = {isLoading: true, isFailed: false, inSuccess: false, ItemDetails: {}}
  ...
  fetchCourseDetails = async () => {
    ...
    if (response.ok) {
      ...
      this.setState({isLoading: false, inSuccess: false, isFailed: true})
    }
  }

  render() {
    const {isLoading, isFailed, inSuccess, ItemDetails} = this.state
